...
const QUALITY_DEFAULTS = {
  high: { model: "large-v3-turbo", compute_type: "", beam_size: "5" },
  accurate: { model: "medium", compute_type: "", beam_size: "8" },
  cpu_safe: { model: "medium", compute_type: "int8", beam_size: "5" },
  fast: { model: "small", compute_type: "", beam_size: "5" },
};
...
function formPayload(form) {
  ...
  Object.keys(data).forEach((key) => { if (data[key] === "" && !["proxy", "remote_components"].includes(key)) delete data[key]; });
  return data;
}

function loadLocalSecrets() {
  const sessionKey = "liveTranscriber.geminiKey.v2";
  const inputs = [$("geminiApiKey"), $("pipelineGeminiApiKey")].filter(Boolean);
  for (const input of inputs) {
    input.value = sessionStorage.getItem(sessionKey) || localStorage.getItem(sessionKey) || localStorage.getItem(GEMINI_KEY_STORAGE) || "";
    input.addEventListener("input", () => {
      sessionStorage.setItem(sessionKey, input.value.trim());
      for (const peer of inputs) if (peer !== input) peer.value = input.value;
    });
  }
}
...
async function loadStatus() {
  ...
  if (!state.defaultsLoaded) {
    for (const form of [$("pipelineForm"), $("transcribeForm")]) {
      for (const name of ["quality", "language", "device", "proxy"]) {
        const field = form.elements.namedItem(name);
        if (field && status.defaults?.[name] != null) field.value = status.defaults[name];
      }
    }
...
function renderQuickAction(label, target, cls) {
  if (!target) return "";
  return `<button class="${escapeAttr(cls)}" type="button" data-action="${escapeAttr(target.action || "preview")}" data-path="${escapeAttr(target.path)}" data-subtitle="${escapeAttr(target.subtitle || "")}">${escapeHtml(label)}</button>`;
}

// Paths are data, never JavaScript source (including quotes and backslashes).
...
async function loadJobs() {
  ...
  const anyRunning = jobs.some((job) => ["running", "pending", "stopping"].includes(job.status));
  ...
  $("jobs").innerHTML = visibleJobs.map((job) => `
    <div class="job ${escapeAttr(job.status)}">
      ...
      <div class="job-actions">
        <button type="button" data-action="log" data-job-id="${escapeAttr(job.job_id)}">日志</button>
        ${["running", "pending"].includes(job.status) ? `<button type="button" class="stop-btn" data-action="stop" data-job-id="${escapeAttr(job.job_id)}">停止</button>` : ""}
      </div>
...
async function pollJobs() {
  try { await loadJobs(); } catch (error) { console.warn(error.message); }
